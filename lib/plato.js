...
exports.inspect = function(files, outputDir, options, done) {
  ...
  var flags = {
    complexity : { // default complexity options
      logicalor : true,
      switchcase : true,
      forin : false,
      trycatch : false,
...
exports.getOverviewReport = function (reports) {
  ...
  reports.forEach(function(report) {
    ...
    var reportItem = {};
    var lintReport = report.jshint || report.eslint;
    reportItem.info = report.info;
    if (lintReport) {
      summary.total.lint = (summary.total.lint || 0) + lintReport.messages.length;
      reportItem.lint = {
        messages : lintReport.messages.length
      };
    }
  ...
  if (summary.total.lint) {
    summary.average.lint = (summary.total.lint / reports.length).toFixed(2);
  }
